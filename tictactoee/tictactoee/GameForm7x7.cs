...
namespace TicTacToe
{
    public partial class GameForm7x7 : Form
    {
        private int boardSize = 7; // 7x7 tahtası
        ...
        private bool CheckForWinner()
        {
            ...
            {
                for (int j = 0; j <= boardSize - 3; j++) // Sağ çaprazlar için döngü
                {
                    // Sol üstten sağ alta çapraz
                    if (CheckLine(boardState[i, j], boardState[i + 1, j + 1], boardState[i + 2, j + 2])) return true;

                    // Sağ üstten sol alta çapraz
                    if (CheckLine(boardState[i, boardSize - 1 - j], boardState[i + 1, boardSize - 2 - j], boardState[i + 2, boardSize - 3 - j])) return true;
                }
            }
